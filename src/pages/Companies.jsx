  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Find sports organizations actively hiring on Staff the Game.</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-athleticBlue"
          />
        </div>
        <select
          value={sportFilter}
          onChange={(e) => setSport(e.target.value)}
          className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-athleticBlue"
        >
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-500">
          ...
          <Link to="/register?role=employer" className="inline-block mt-4 text-athleticBlue font-medium hover:underline">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((c) => (
            <Link
              key={c.id}
              to={`/companies/${c.id}`}
              className="group bg-white border border-gray-200 rounded-2xl p-6 hover:border-athleticBlue hover:shadow-card-hover transition-all shadow-card"
            >
              <div className="flex items-start gap-4 mb-4">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    <h3 className="font-semibold text-gray-900 group-hover:text-athleticBlue truncate">{c.name}</h3>
                    {c.verified && <CheckCircle className="w-4 h-4 text-energyGreen shrink-0" />}
                  </div>
